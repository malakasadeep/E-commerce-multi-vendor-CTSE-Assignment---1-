...
export default function OrdersPage() {
  ...
  return (
    <div className="w-[90%] max-w-7xl mx-auto py-8">
      <h1 className="text-2xl font-bold text-gray-900 font-Poppins mb-6">My Orders</h1>
              ...
              <div className="bg-white rounded-xl border p-5 hover:shadow-md transition-shadow cursor-pointer">
                ...
                <div className="flex items-center gap-4">
                  <div className="flex -space-x-2">
                    {order.items.slice(0, 3).map((item: any, i: number) => (
                      item.product?.images?.[0]?.url ? (
                        <img key={i} src={item.product.images[0].url} alt="" className="w-10 h-10 rounded-lg border-2 border-white object-cover" />
                      ) : (
                        <div key={i} className="w-10 h-10 rounded-lg border-2 border-white bg-gray-100 flex items-center justify-center">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-xs text-gray-500">incl. ${order.serviceFee.toFixed(2)} fee</p>
                  </div>
                </div>
              </div>
            </Link>
