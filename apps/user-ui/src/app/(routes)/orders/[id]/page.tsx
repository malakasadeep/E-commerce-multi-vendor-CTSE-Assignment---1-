  ...
  return (
    <div className="w-[90%] max-w-5xl mx-auto py-8">
      <div className="flex items-center gap-3 mb-6">
        ...
        <Badge className={`ml-auto ${STATUS_COLORS[order.status]} text-sm px-3 py-1`}>{order.status}</Badge>
      </div>
      ...
      {isCancelled && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-6 mb-6 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-red-600">This order has been {order.status}.</p>
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div className="bg-white rounded-xl border p-5">
          ...
          <p className="text-sm text-gray-600">{order.shippingAddress?.zip} {order.shippingAddress?.country}</p>
        </div>
        <div className="bg-white rounded-xl border p-5">
          <div className="flex items-center gap-2 mb-3">
          ...
          <p className="text-sm text-gray-600">
            Status: {order.payment ? (
              <Badge className={STATUS_COLORS[order.payment.status] || 'bg-gray-100'}>{order.payment.status}</Badge>
            ) : 'Awaiting payment'}
          </p>
          <p className="text-sm text-gray-600 mt-1">Amount: ${order.total.toFixed(2)}</p>
        </div>
      </div>
            ...
            <div key={item.id} className="flex items-center gap-4 p-3 bg-gray-50 rounded-lg">
              ...
              <div className="flex-1">
                ...
                {item.product?.shop?.name && (
                  <p className="text-xs text-gray-400">from {item.product.shop.name}</p>
                )}
              </div>
              <div className="text-right">
                <p className="font-semibold">${(item.price * item.quantity).toFixed(2)}</p>
                <Badge className={`mt-1 ${STATUS_COLORS[item.status] || 'bg-gray-100'}`}>{item.status}</Badge>
              </div>
            </div>
          ))}
        </div>
